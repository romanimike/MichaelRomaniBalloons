...
(function () {
  ...
  /* ---------------------------------------------------------------
     Mini gallery scroll row
  --------------------------------------------------------------- */
  document.querySelectorAll("[data-scroll-row]").forEach(function (root) {
    var track = root.querySelector(".review-track-wrap");
    var prevBtn = root.querySelector(".review-arrow.prev");
    var nextBtn = root.querySelector(".review-arrow.next");
    if (!track) return;

    function scrollByPage(dir) {
      track.scrollBy({ left: dir * track.clientWidth * 0.85, behavior: reduceMotion.matches ? "auto" : "smooth" });
    }
